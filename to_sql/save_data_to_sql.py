...
class Save_score_to_sql(object):
    ...
    def get_keyword(self):
        ...
        sql = 'select search_keyword,spider_status,section_name,author from basic_information where spider_status=0 and channel_name=3'
        try:
            self.cur.execute(sql)
    ...
    def undate_data(self, status_, keyword):
        # 更改爬虫配置表状态
        sql = "UPDATE basic_information SET spider_status = '{}' WHERE search_keyword = '{}' and  channel_name=3".format(
            status_, keyword)
        try:
            # print(sql)
            # 执行SQL语句
